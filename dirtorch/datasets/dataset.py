...
class Dataset(object):
    ...
    def get_key(self, img_idx):#需要子类实现这个方法
        raise NotImplementedError()

    def key_to_index(self, key):#idx指示图片的索引，key是图片的文件名
        if not hasattr(self, '_key_to_index'):
            self._key_to_index = {self.get_key(i):i for i in range(len(self))}
        return self._key_to_index[key]

    def get_image(self, img_idx, resize=None):#返回一个元组 (width, height)
    ...
    def get_image_size(self, img_idx):#这个方法好像没用
        return self.imsize
    ...
    def eval_query_top(self, query_idx, scores, k=(1,5,10,20,50,100)):#按照分类标签来比较的

...
def extract_image_features(dataset, transforms, net, ret_imgs=False, same_size=False, flip=None,
    ...
    loader = get_loader(dataset, trf_chain=transforms, preprocess=net.preprocess, iscuda=iscuda,
                        output=['img'], batch_size=batch_size, threads=threads, training=True, shuffle=False)
    device=torch.device('cuda' if args.iscuda else 'cpu')
    cnt=0
    net.eval()
    with torch.no_grad():
        for inputs in tqdm.tqdm(loader, desc="Training...", total=1+(len(dataset)-1)//batch_size):
            cnt+=1
    #        print(type(inputs))
...
def eval_model(db, net, trfs, pooling='mean', gemp=3, detailed=False, whiten=None,
               ...
               load_feats=None, dbg=()):#修改了threads
...
def load_model(path, iscuda):
    ...
    net = nets.create_model(pretrained="", **checkpoint['model_options'])#此处pretrained可选'imagenet'
...
#    parser.add_argument('--checkpoint', type=str, required=True, help='path to weights')
    ...
    parser.add_argument('--threads', type=int, default=8, help='number of thread workers')#修改了threads
    parser.add_argument('--gpu', type=int, default=3, nargs='+', help='GPU ids')

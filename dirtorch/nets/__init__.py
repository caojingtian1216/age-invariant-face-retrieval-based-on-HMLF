...
def create_model(arch, pretrained='', delete_fc=False, *args, **kwargs):#注意顺序
    ...
    if os.path.isfile(pretrained or ''):
        ...
        #weights = torch.load(pretrained, map_location=lambda storage, loc: storage)['state_dict']#改了一下
        #print(type(weights))
        weights = torch.load(pretrained, map_location=lambda storage, loc: storage)
        load_pretrained_weights(model, weights, delete_fc=delete_fc)#区别：这是函数

    elif pretrained:#pretrained可以='imagenet'
        assert hasattr(model, 'load_pretrained_weights'), 'Model %s must be initialized with a valid model file (not %s)' % (arch, pretrained)
        model.load_pretrained_weights(pretrained)#区别：这是类的方法（没有删除fc层的选项）
...
def load_pretrained_weights(net, state_dict, delete_fc=False):
    ...
    for k, v in list(state_dict.items()):#先统一去掉module

...
class SwinTransformer(nn.Module):
    ...
    def flops(self):
        ...
        return flops
    '''
# 在 SwinTransformer 类的 forward 方法之后添加：

    def load_pretrained_weights(self, pretrained_path):
        """
        加载官方预训练权重，只加载backbone部分，跳过output_layer
        ...
        # 过滤掉不匹配的键（主要是output_layer和head相关的层）
        pretrained_dict = {}
        skipped_keys = []
        loaded_keys = []
...
# 然后修改函数定义部分（替换原来的三个函数）：

def swin_tiny_patch4_window7_224(pretrained=None, **kwargs):
    """Swin-T (Swin-Tiny) model @ 224x224
...
def swin_small_patch4_window7_224(pretrained=None, **kwargs):
    """Swin-S (Swin-Small) model @ 224x224
    ...
    model.rgb_means = [0.485, 0.456, 0.406]
    model.rgb_stds = [0.229, 0.224, 0.225]
    model.input_size = (3, 224, 224)
    model.fc_name = 'output_layer'
...
def swin_base_patch4_window7_224(pretrained=None, **kwargs):
    """Swin-B (Swin-Base) model @ 224x224

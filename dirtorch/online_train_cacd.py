...
def visualize_retrieval_results_grid(query_images, retrieval_results, save_path=None):
    """
    可视化图像检索结果，支持多个查询
    
    参数:
        query_images: 查询图像列表 [(image, is_correct), ...]
        retrieval_results: 检索结果列表，每个查询对应10个检索结果 
                          [[(image, is_correct), ...], ...]
        save_path: 保存路径，如果为None则直接显示
    """
    n_queries = len(query_images)
    
    # 创建画布 - 调整大小以适应224x224的图像
    fig = plt.figure(figsize=(20, n_queries * 2.5))
    
    # 使用GridSpec来精确控制布局
    # 第一列(Query)宽度为1，间隔为0.3，其余10列紧凑排列
    width_ratios = [1, 0.3] + [1] * 10  # Query列 + 间隔 + 10个结果列
    
    from matplotlib.gridspec import GridSpec
    gs = GridSpec(n_queries, 12, figure=fig, 
    ...
    # 添加顶部标题 "Query"
    fig.text(0.09, 0.96, 'Query', ha='center', va='top', 
             fontsize=18, fontweight='bold')
    
    # 添加顶部标题 "Rank-1→Rank-10" 和箭头
    arrow_start_x = 0.22
    arrow_end_x = 0.96
    arrow_y = 0.96
    
    # 添加文字 "Rank-1→Rank-10"
    fig.text((arrow_start_x + arrow_end_x) / 2, 0.96, 
            'Rank-1→Rank-10', ha='center', va='top', 
            fontsize=18, fontweight='bold')
    
    for i in range(n_queries):
        # 绘制查询图像 (第0列)
        query_img = query_images[i]
        ax_query = fig.add_subplot(gs[i, 0])
        ax_query.imshow(query_img, cmap='gray' if len(query_img.shape) == 2 else None)
        ax_query.axis('off')
        
        # 绘制查询图像边框 - 黑色粗边框
        rect = patches.Rectangle((0, 0), query_img.shape[1]-1, query_img.shape[0]-1,
                                linewidth=4, edgecolor='black', facecolor='none')
        ax_query.add_patch(rect)
        
        # 绘制分割线 (第1列 - 虚线)
        ax_divider = fig.add_subplot(gs[i, 1])
        ax_divider.axis('off')
        # 在分割列中间画一条垂直虚线
        ax_divider.axvline(x=0.5, color='black', linestyle='--', linewidth=2)
        
        # 绘制检索结果 (第2-11列，紧凑排列)
        for j in range(10):
            ax_result = fig.add_subplot(gs[i, j+2])
            
            if j < len(retrieval_results[i]):
                ret_img, is_correct = retrieval_results[i][j]
                ax_result.imshow(ret_img, cmap='gray' if len(ret_img.shape) == 2 else None)
                ax_result.axis('off')
                
                # 根据正确性绘制边框
                color = 'green' if is_correct else 'red'
                rect = patches.Rectangle((0, 0), ret_img.shape[1]-1, ret_img.shape[0]-1,
...
#    net=get_mtl_model(backbone_name='irse50')
...
#    criterion=TripletLoss(margin=0.3, hard_mining=False)#在线Hard Triplet Loss
...
#    optim=Adam(net.parameters(),lr=0.0001)
    ...
    random_indices=random.sample(range(1200), 4)
    
    dataset = datasets.create('CADA2000_trainvalid')

    img_1=dataset.get_image(1)
    ...
    for idx in range(3):
        ...
        visualize_retrieval_results(3,qdataset.get_image(idx),retrieval_images,res['scores'][idx],labels,qlabel)
    ...
    for key, value in res.items():
        ...
        if count == 2:
            break
    eval_map3.append(res['mAP'])
    p3.append(res['P@1'])

    retrieval_results=[]
    query_images=[]
    for idx in random_indices:
        retrieval_result=[(np.array(dataset.get_image(i)),dataset.index2[i]==dataset.index[idx]) for i in res['results'][idx]]
        labels=[dataset.index2[i] for i in res['results'][idx]]
        qlabel=dataset.index[idx]
        retrieval_results.append(retrieval_result)
        query_images.append(np.array(qdataset.get_image(idx)))
    visualize_retrieval_results_grid(query_images,retrieval_results,save_path='/kaggle/working/retrieval_results_3.jpg')
    ...
    for idx in random_indices:
        ...
        retrieval_results.append(retrieval_result)
        query_images.append(np.array(qdataset.get_image(idx)))
    visualize_retrieval_results_grid(query_images,retrieval_results,save_path='/kaggle/working/retrieval_results_2.jpg')
    ...
    for idx in random_indices:
        ...
        retrieval_results.append(retrieval_result)
        query_images.append(np.array(qdataset.get_image(idx)))
    visualize_retrieval_results_grid(query_images,retrieval_results,save_path='/kaggle/working/retrieval_results_1.jpg')
...
#                i.requires_grad = True
        ...
        for batch_idx, (data, labels) in tqdm.tqdm(enumerate(loader), total=1000):
            ...
            #loss = 0.1*loss_arc + loss_nce
            loss=criterion_arc(embeddings, labels)
            
            # 反向传播
            loss.backward()

            #torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=1.0,norm_type=2)#梯度裁剪，防止梯度爆炸
            #torch.nn.utils.clip_grad_norm_(criterion.parameters(), max_norm=1.0,norm_type=2)
            ...
            if time.time()-start_time>3600*3.5:
                ...
                plot_map(train_map, eval_map1, eval_map2, eval_map3)
                plot_p(train_p, p1, p2, p3)
                print(f"即将超时（已运行 {elapsed/3600:.2f} 小时），开始清理...")
    ...
    #    print(' * ' + '\n * '.join(['%s = %g' % p for p in res.items()]))
        ...
        for idx in range(3):
            ...
            labels=[dataset.index2[i] for i in res['results'][idx]]
            qlabel=dataset.index[idx]
            visualize_retrieval_results(2,qdataset.get_image(idx),retrieval_images,res['scores'][idx],labels,qlabel)

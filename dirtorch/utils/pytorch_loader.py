...
def get_loader( dataset, trf_chain, iscuda,
    ...
    if training == True:
        loader = PytorchLoader2(dataset, transform=trf_chain, output=output,**preprocess)
    else:
        loader = PytorchLoader(dataset, transform=trf_chain, output=output,**preprocess)
    ...
    if threads == 1:
        ...
        return data.DataLoader(
            ...
            sampler = sampler,
            num_workers = threads,
            pin_memory = iscuda)#pin_memory=True,GPU上的传输


#再写一个pytorchloader
class PytorchLoader2(data.Dataset):
    """A pytorch dataset-loader

     Args:
        dataset (object):  dataset inherited from dataset.Dataset()

        transform (deprecated, callable): pytorch transforms. Use img_and_target_transform instead.#已弃用

        target_transform (deprecated, callable): applied on target. Use img_and_target_transform instead.#已弃用
    ...
    def __init__(self, dataset, transform=None,
                       ...
                       output=['img','label'],**preprocess):
        self.dataset = dataset
        self.transform = transform
        from . import transforms
        print(preprocess)
        self.transform = transforms.create("tvf.Resize(160),tvf.RandomCrop(160)", to_tensor=True, **preprocess)
        self.target_transform = target_transform
        self.img_and_target_transform = img_and_target_transform
        self.output = output
        self.rank_lst=[list(range(20+50*i,50+50*i)) for i in range(40)]#***改过了，原来这几行没有
        self.rank_lst=np.array(self.rank_lst).reshape(-1)#***

    def __getitem__(self, index):
        img_filename = self.dataset.get_filename(index)
#        print((index,np.random.choice(self.dataset.get_relevants(index),1)[0],self.dataset.get_irrelevants(index)))
#        print(index)
        index=index % 1200#***
        pos_choice=np.random.choice(self.dataset.get_relevants(self.rank_lst[index]),2,replace=False)#***
        neg_choice=np.random.choice(self.dataset.get_irrelevants(self.rank_lst[index]),1)

        img_and_label = dict(
            img_filename = img_filename,
            img_key   = self.dataset.get_key(index),
            img   = self.dataset.get_image(index),
            img_tuple=[self.dataset.get_image(pos_choice[0]),
            self.dataset.get_image(pos_choice[1]),
            self.dataset.get_image(neg_choice[0])],
        ...
        if self.img_and_target_transform:
            ...
            for out_key in self.output:
                for type in ['_instance_map', '_angle_map']:
                    #_instance_map（实例映射）：实例映射是一种标签映射，用于区分图像中的不同实例（即使它们属于同一类别）。每个不同的实例通常由一个唯一的整数标识。
                    #_angle_map（角度映射）：角度映射是一种标签映射，用于表示图像中每个像素或区域的方向或角度信息。这种映射通常用于需要方向信息的任务，如纹理分析、形状分析或某些特定的物体检测任务。
                    #label_map（标签映射）：标签映射可以用于表示图像中的不同物体，如行人、车辆、道路等，其中每个类别由一个唯一的整数表示。
...
#                img_and_label['img'] = self.transform(img_and_label['img'])
                img_and_label['img_tuple'] = [self.transform(img_and_label['img_tuple'][0]),
                self.transform(img_and_label['img_tuple'][1]),
                self.transform(img_and_label['img_tuple'][2])]
                img_and_label['img']=img_and_label['img_tuple']
...
class PytorchLoader (data.Dataset):
    ...
    def __init__(self, dataset, transform=None,
                       target_transform=None,
                       img_and_target_transform=None,
                       output=['img','label'],**preprocess):#改过了，原来没有preprocess
        self.dataset = dataset
        from . import transforms
#        print(preprocess)
        self.transform = transforms.create(f"tvf.Resize({cfg.INPUT_SIZE}),tvf.RandomCrop({cfg.INPUT_SIZE})", to_tensor=True, **preprocess)
        self.target_transform = target_transform
        self.img_and_target_transform = img_and_target_transform
        self.output = output
...
class BalancedSampler (data.sampler.Sampler):
    ...
    size:   float in [0,2]
        ...
        **will have less items, but conversely, under-classes will have more items.**看看这句话
    ...
    def __init__(self, dataset, size=1.0, balanced=1.0, use_all=False):#size控制了百分位数
        assert 0 <= size <= 2
        assert 0 <= balanced <= 1
        ...
        for i in range(len(dataset)):
            label = dataset.get_label(i, toint=True)#toint=True时将标签（可能是字符）转化为整数，便于表示和计算
            self.cls_imgs[label].append(i)

        # decide on the number of example per class
        self.npc = np.percentile([len(imgs) for imgs in self.cls_imgs], max(0,min(50*size,100)))#如果 size 是 0.5，那么 self.npc 将是图像数量列表的第 25 百分位数，这可以用来表示图像数量较少的类别的图像数量
    ...
    def __iter__(self):
        ...
        for i,imgs in enumerate(self.cls_imgs):
            ...
            res = []
            while len(res) < target:#可能同一张图片被加了多次，增强类别，过采样（Oversampling）：对样本数量较少的类别进行复制，以增加其在数据集中的代表性。
                res += imgs
            res = res[:target] # cut

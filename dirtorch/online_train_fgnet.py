...
def print_res(res):
    ...
    for key, value in res.items():
        ...
        if count == 2:
            break
def prepare_work():
    FGNET_ROOT=r"/kaggle/working/deep-image-retrieval-master/dirtorch/data/datasets/FGNET/jpg"
    ...
    return df
def create_pkl_files(img_id, df):
    #先制作gnd_fgnet
    ...
    path=r'/kaggle/working/deep-image-retrieval-master/dirtorch/data/datasets/FGNET/gnd_fgnet.pkl'
    with open(path, 'wb') as f:
        pkl.dump(gnd_fgnet, f)
    ...
    for i in range(82):
        ...
        gnd_fgnet['gnd'][i]['irrel']=irrel_list
    #print(gnd_fgnet['imlist'][gnd_fgnet['gnd'][2]['ok']])
    path=r'/kaggle/working/deep-image-retrieval-master/dirtorch/data/datasets/FGNET/fgnet_train.pkl'
    with open(path, 'wb') as f:
        pkl.dump(gnd_fgnet, f)
    ...
    for i in range(50):
        ...
        irrel_list=np.where(train_label!=test_label[i])[0]
        gnd_fgnet['gnd'][i]['irrel']=irrel_list
    path=r'/kaggle/working/deep-image-retrieval-master/dirtorch/data/datasets/FGNET/fgnet_trainvalid.pkl'
    with open(path, 'wb') as f:
        pkl.dump(gnd_fgnet, f)
...
#    parser.add_argument('--whiten', type=str, default='Landmarks_clean', help='applies whitening')
    ...
    train_idx=[0, 15, 31, 43, 55, 66, 78, 87, 102, 115, 127, 141, 156, 168, 178, 191, 204, 217, 228, 238, 251, 263, 276, 288, 299, 311, 322, 333, 344, 357, 368, 381, 393, 404, 417, 431, 444, 456, 470, 484, 498, 508, 521, 532, 542, 555, 568, 582, 598, 608, 616, 627, 638, 651, 664, 672, 680, 690, 701, 710, 722, 735, 747, 757, 763, 778, 790, 800, 810, 821, 832, 845, 859, 875, 891, 901, 919, 935, 951, 965, 979, 991]
    ...
    criterion_arc=ArcFaceLoss(embedding_dim=512, num_classes=81)
    criterion_nce = criterion_nce.to(device) # <-- 添加这一行，将损失函数移动到GPU
    criterion_arc = criterion_arc.to(device) # <-- 添加这一行，将损失函数移动到GPU

    all_maps_eval=np.zeros(602)
    all_p1s_eval=np.zeros(602)
    all_maps_train=np.zeros(602)
    all_p1s_train=np.zeros(602)
#    dl.download_dataset(args.dataset)
#    dataset = datasets.create('Paris6K_train')
    df=prepare_work()
    for img_id in range(602):#原来是1002
        print(f'Processing image ID: {img_id}')
        create_pkl_files(img_id, df)
    ...
    #    optim=Adam(net.parameters(),lr=0.0001)
        ...
        dataset = datasets.create('fgnet_trainvalid')
        ...
        all_maps_train[img_id]=res['mAP']
        all_p1s_train[img_id]=res['P@1']
        
        dataset = datasets.create('fgnet')
        qdataset=dataset.get_query_db()
        net.eval()
        args.detailed=False
        res = eval_model(dataset, net, args.trfs, pooling=args.pooling, gemp=args.gemp, detailed=args.detailed,
        ...
        all_maps_eval[img_id]=res['mAP']
        all_p1s_eval[img_id]=res['P@1']
        '''
        if img_id in train_idx:
            net.train()
            for epoch in range(Epoch_num):
                ...
                dataset = datasets.create('fgnet_train')#创建数据集对象
                net.train()
                loader = create_pk_dataloader(dataset, p=8, k=4, num_workers=args.threads)#创建P-K采样的数据加载器
                device=torch.device('cuda' if args.iscuda else 'cpu')
                cnt=0
                Ave_loss=num_batches=0
                for batch_idx, (data, labels) in tqdm.tqdm(enumerate(loader), total=1001/(8*4)):
                     ...
                     #   elapsed = time.time() - start_time
                      #  plot_map(train_map, eval_map)
                       # plot_p(train_p, eval_p)
                        #print(f"即将超时（已运行 {elapsed/3600:.2f} 小时），开始退出...")
                        #break
            ...
            #    dataset = datasets.create('Paris6K')
                ...
                train_map.append(res['mAP'])
                train_p.append(res['P@1'])
                all_maps_train[img_id]=train_map[-1]
                all_p1s_train[img_id]=train_p[-1]
            ...
            #    print(' * ' + '\n * '.join(['%s = %g' % p for p in res.items()]))
                ...
                all_maps_eval[img_id]=eval_map[-1]
                all_p1s_eval[img_id]=eval_p[-1]
    ...
    print("前二十张测试集图片的mAP：", all_maps_eval[:20])
    print("前二十张测试集图片的P@1：", all_p1s_eval[:20])
    print("前二十张训练集图片的mAP：", all_maps_train[:20])
    print("前二十张训练集图片的P@1：", all_p1s_train[:20])
    print("平均测试集mAP：", np.mean(all_maps_eval))
    print("平均测试集P@1：", np.mean(all_p1s_eval))
    cleanup_and_save(net, optim)

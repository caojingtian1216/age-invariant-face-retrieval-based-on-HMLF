...
#    criterion=TripletLoss(margin=0.3, hard_mining=False)#在线Hard Triplet Loss
    ...
    criterion_arc=ArcFaceLoss(embedding_dim=512, num_classes=5000)#假设有5000个人脸类别
...
#    optim=Adam(net.parameters(),lr=0.0001)
    ...
    dataset = datasets.create('imdb_trainvalid')
    ...
    for epoch in range(Epoch_num):
        ...
        dataset = datasets.create('imdb_train')#创建数据集对象
        net.train()
        p,k=16,4
        ...
        for batch_idx, (data, labels) in tqdm.tqdm(enumerate(loader), total=2304/(p*k)):#2304是morph_train的图片数量
            ...
            loss=criterion_nce(embeddings, labels)
